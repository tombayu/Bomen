...
trees <- readOGR("C:/Users/tombayu.hidayat/Documents/Internship/Shiny/data/KroonBufferNew.json")
...
max1 <- list()
for (i in 1: ncol(lai1Trees)) {
  max1[i] <- max(lai1Trees[, i])
}
max(unlist(max1))
min(unlist(max1))

max2 <- list()
for (i in 1: ncol(lai2Trees)) {
  max2[i] <- max(lai2Trees[, i])
}
max(unlist(max2))
min(unlist(max2))

max3 <- list()
for (i in 1: ncol(lai3Trees)) {
  max3[i] <- max(lai3Trees[, i])
}
max(unlist(max3))
min(unlist(max3))

trees@data$LAI1 <- unlist(unlist(max1))
trees@data$LAI2 <- unlist(unlist(max2))
trees@data$LAI3 <- unlist(unlist(max3))

# Save the maximum values
writeOGR(trees, dsn = "Supplementary/treesLAI.geojson", layer = "LAI", driver = "GeoJSON")
writeOGR(trees, dsn = "Supplementary/treesLAI.shp", layer = "LAI", driver = "ESRI Shapefile")
